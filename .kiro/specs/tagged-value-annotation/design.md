...
When `@Constructor` or `@Invoker` generates stage methods (e.g., `.userId(10).firstName("John")`), the tag annotations on the original constructor/method parameters are propagated to the generated stage method parameters. The `TaggedValueAnalyzer` inspects the arguments passed to these stage methods and applies the same tag-checking rules using the tag on the original parameter as the target tag.

This works because:
- The `TaggedValueAnalyzer` resolves the target tag by looking up the parameter declaration of the method being called.
- Generated stage methods have parameters that correspond 1:1 to the original annotated parameters.
- The tag annotations are available at the declaration site of the original constructor/method parameter, which the analyzer can trace through the generated code.
...
- **TaggedValueAnalyzerIntegrationTest** — End-to-end integration tests that compile test source files with `@TaggedValue` tag annotations and verify the correct warnings are emitted. Covers Requirements 10.1, 10.2 (builder chain interaction), 13.1–13.3 (processor integration).
